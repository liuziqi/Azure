...
void on_request_method(void *data, const char *at, size_t length) {
    ...
    if(m == HttpMethod::HTTP_INVALID_METHOD) {
        AZURE_LOG_WARN(g_logger) << "invalid http request method: " << std::string(at, length);
...
void on_request_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        AZURE_LOG_WARN(g_logger) << "invalid http request version: " << std::string(at, length);
...
// 已处理的字节数，且data有效数据为 len - v
size_t HttpRequestParser::execute(char *data, size_t len) {
    size_t offset = http_parser_execute(&m_parser, data, len, 0);
    memmove((void*)data, data + offset, (len - offset));    // 将缓冲区的内容向后移
    return offset;
}
...
void on_response_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        AZURE_LOG_WARN(g_logger) << "invalid http response version: " << std::string(at, length);
        parser->setError(1001);
        return;
    }
    parser->getData()->setVersion(v);
