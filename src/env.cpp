...
bool Env::init(int argc, char **argv) {
    char link[1024] = {0};
    char path[1024] = {0};
    sprintf(link, "/proc/%d/exe", getpid());     // 软链接路径
    readlink(link, path, sizeof(path));         // 软链接转换为实际路径
    m_exe = path;                               // /path/xxx/exe
    
    auto pos = m_exe.find_last_of("/");
    m_cwd = m_exe.substr(0, pos + 1);           // /path/xxx/

    m_program = argv[0];
    // -config /path/to/cpnfig -file xxxx -d
    ...
    for(int i = 1; i < argc; ++i) {
        if(argv[i][0] == '-') {
            if(strlen(argv[i]) > 1) {
                ...
                AZURE_LOG_INFO(g_logger) << "invalid arg idx=" << i << " val=" << argv[i];
                return false;
            }
        }
        else {
            if(now_key) {
                ...
                AZURE_LOG_INFO(g_logger) << "invalid arg idx=" << i
                                         << " val=" << argv[i];
                return false;
            }
        }
